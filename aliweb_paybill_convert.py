...
def convert_aliweb_paybill_to_standard_accountlist(df, info_data):
    """
    原始数据:
    ...
    # 完成列名修改、增加新列和重新排序
    df = df.rename(columns={"付款时间": "日期", "商品名称": "商品", "金额（元）": "金额", "收/支": "交易类型", "服务费（元）": "手续费"})
...
def aliweb_paybill_auto_classify(df, info_data):
    ...
    df = auto_calssify_by_keyword(df, first_classify_col='一级分类名称',
                                  second_classify_col='二级分类名称', match_list_rule=info_data.classify_csv_rule)
...
def aliweb_paybill_conv_dev(df, info_data, dst_app):
    # 原始账单格式
